...
if [ -f "$HOME/.build" ]; then 
	...
	#
	cd ..
	tar zcf $HOME/tezos-$branch.tar.gz tezos/tezos-* tezos/active_protocol_versions tezos/active_testing_protocol_versions tezos/sc_rollup_protocol_versions tezos/tx_rollup_protocol_versions tezos/script-inputs/*
